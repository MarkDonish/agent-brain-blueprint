...
demo-doctor:
	...
	# Claimed path: Codex owns current_work → must report a conflict
	$(PYTHON) -c 'import json,subprocess,sys; p=subprocess.run([sys.executable,"scripts/check_claim_gate.py","examples/demo-vault","--path","10_projects/demo-notes-app/10_current_work/INDEX.md"],capture_output=True,text=True); r=json.loads(p.stdout); assert r.get("allowed") is False and r.get("conflict_count",0)>=1, r; print("demo claim conflict ok")'
